...
{	var $tipo_ambiente= 1;	var $tipo_emision= 1;		var $ruta_archivo_xml= "";		var $ruta_archivo_pdf= "";		var $CompElectronico= "";
    ...
    function ManejoXml($nombre_archivo, $arr_datos, $tag)
    {
        ...
        shell_exec("java -jar ".$app_java." ".$ruta_de_la_firma." ".$clave_de_la_firma." ".$this->ruta_archivo_xml." ".$ruta_carpeta_xml);
    ...
    function WSSRI($claveAcceso)
    {
        ...
        if(!$this->consulta_autoriz):
            ...
            $oSoapClient = new nusoap_client($ws_recepcion_comprobante, true);
            $oSoapClient->soap_defencoding = 'utf-8';

            $contenido_xml= base64_encode(file_get_contents($this->ruta_archivo_xml));
            set_time_limit(120);
            $respuesta = $oSoapClient->call("validarComprobante", array("xml" => $contenido_xml));
            $sError = $oSoapClient->getError();
            //christian alvarado
            $ERROR = $respuesta[RespuestaRecepcionComprobante][comprobantes][comprobante][mensajes][mensaje];
            if($ERROR[identificador] != "43"){ //$ERROR[tipo] == "ERROR" &&
                //goto GET_AUTORIZACION;
                if($oSoapClient->fault):
                    $this->log_errores("OPERACION", $sError);
                    return false;
                else:
                    if($sError):
                        $this->log_errores("CONEXION", $sError);
                        return false;
                    endif;
                endif;
                ...
                if($resp_sri["estado"]!="RECIBIDA"):

                    $resp_sri = $resp_sri["comprobantes"]["comprobante"];
                    if(isset($resp_sri["mensajes"]["mensaje"][0])):
                        $this->log_errores("RECEPCIÓN XML", $resp_sri["mensajes"]["mensaje"][0]["mensaje"]."\n\n".$resp_sri["mensajes"]["mensaje"][0]["informacionAdicional"]);
                    else:
                        $this->log_errores("RECEPCIÓN XML", $resp_sri["mensajes"]["mensaje"]["mensaje"]."\n\n".$resp_sri["mensajes"]["mensaje"]["informacionAdicional"]);
        ...
        //Si el error es 43 saltar hasta aqui
        //GET_AUTORIZACION:
        ...
        $respuesta = $oSoapClient->call("autorizacionComprobante", array("claveAccesoComprobante" => $claveAcceso));
        $sError = $oSoapClient->getError();
        if($oSoapClient->fault):
            $this->log_errores("OPERACION", $sError);
            return false;
    ...
    function generar_pdf($nombre_archivo, $arreglo)
    {
        ...
        //<p>AGENTE DE RETENCION SEGUN RESOLUCION No. NAC-DNCRASC20-00000001</p>
        $html .= "									
					                <p>OBLIGADO A LLEVAR CONTABILIDAD: ".$arreglo["infoCorreo"]["obligadoContabilidad"]."</p>
								</td>
							</tr>
					...
					<tr>
						<td style='vertical-align:top'>
							<table style='width: 100%' border='0.5px' cellpadding='0' cellspacing='0'>
								...
								<tr>
									<td align='center'>20 - OTROS CON UTILIZACIÓN DE SISTEMA FINANCIERO </td>
        ...
        $html.=	"	<tr><td><p>Razón Social / Nombres y Apellidos:</p></td>
									...
									<tr><td><p>Dirección:</p></td>
										<td><p>".$arreglo["infoCorreo"]["direccionComprador"]."</p></td>
									</tr>";
        if($this->tipo_doc!='06'):
        ...
        if($this->tipo_doc=='01' or $this->tipo_doc=='04' or $this->tipo_doc=='06'):
            if($this->tipo_doc=='06'):
                $html.= "	<col style='width: 10%'>
											...
											<tr><td align='center' height='40'><b>Cantidad</b></td>
												...
												<td align='center'><b>Unidad <br>Medida</b></td>
											</tr>";					$detalle= $arreglo["destinatarios"]["destinatario"]["detalles"]["detalle"];
            else:
                $ancho_desc= (40-(count($this->det_adic_pdf)*10)) + 12;
                $html.= "	<col style='width: 12%'>
											
											<col style='width: 8%'>
											<col style='width: $ancho_desc%'>";
                for($i=0; $i<count($this->det_adic_pdf); $i++):		$html.= "	<col style='width: 10%'>";
                endfor;
                $html.= "	<col style='width: 8%'>
											...
											<tr><td align='center' height='40'><b>Cod Principal</b></td>
												...
												<td align='center'><b>Descripción</b></td>";
                for($i=0; $i<count($this->det_adic_pdf); $i++):		$html.= "	<td align='center'><b>".$this->det_adic_pdf[$i]."</b></td>";
                endfor;
                $html.= "	<td align='center'><b>Precio <br>Unitario</b></td>
            ...
            for($i=0; $i<count($detalle); $i++):	$html.= "	<tr>";
                ...
                //$html.= "	<td align='center'>".$detalle[$i]["codigoAuxiliar"]."</td>";
                elseif($this->tipo_doc=='04'):		$html.= "	<td align='center' height='20'>".$detalle[$i]["codigoInterno"]."</td>";

                //$html.= "	<td align='center'>".$detalle[$i]["codigoAdicional"]."</td>";
                elseif($this->tipo_doc=='06'):		$html.= "	<td align='center' height='20'>".$detalle[$i]["cantidad"]."</td>
                    ...
                    $html.= "	<td align='center'>".$detalle[$i]["unidadMedida"]."</td>";
                    //$html.= "	<td align='center'>".$detalle[$i]["codigoAdicional"]."</td>";
                    $html.= "	</tr>";	continue;
                endif;
                $html.= "		<td align='center'>".$detalle[$i]["cantidad"]."</td>
												<td align='left'>".$detalle[$i]["descripcion"]."</td>";
                for($j=0; $j<count($this->det_adic_pdf); $j++):			$encontro= false;				$detAdicional= $detalle[$i]["detallesAdicionales"]["detAdicional"];
                    for($k=0; $k<count($detAdicional); $k++):												//$detAdicional= $detAdicional[$k];
                        if($this->det_adic_pdf[$j]==$detAdicional[$k]["@attributes"]["nombre"]):	$html.= "<td align='center'>".$detAdicional[$k]["@attributes"]["valor"]."</td>";	$encontro= true;	break;
                        endif;
                    endfor;
                    if(!$encontro):				$html.= "<td align='center'></td>";
                    endif;
                endfor;
                $html.= "		<td align='right'>".$detalle[$i]["precioUnitario"]."</td>
												<td align='right'>".$detalle[$i]["descuento"]."</td>
												<td align='right'>".$detalle[$i]["precioTotalSinImpuesto"]."</td>
											</tr>";
            endfor;
        elseif($this->tipo_doc=='07'):
            $html.= "		<col style='width: 12%'>
											...
											<tr><td align='center' height='40'><b>Comprobante</b></td>
												...
												<td align='center'><b>Fecha <br>Emisión</b></td>
												<td align='center'><b>Ejercicio <br>Fiscal</b></td>
            ...
            $detalle= $arreglo["impuestos"]["impuesto"];
            for($i=0; $i<count($detalle); $i++):
                $datos_imp= $this->querys_model->GetDatosImpuesto($cons_where_imp= "AND ref_impuesto='".$detalle[$i]["codigo"]."' AND id_porcentaje='".$detalle[$i]["codigoRetencion"]."'");
                $datos_cmp= $this->querys_model->GetDatosTipoComprobante($cons_where_cmp= "AND codigo='".$detalle[$i]["codDocSustento"]."'");
                if($datos_imp["n_impuesto"]=="RET. FTE."):
                    $datos_imp["n_impuesto"]= "RENTA";
                elseif($datos_imp["n_impuesto"]=="RET. IVA"):
                    $datos_imp["n_impuesto"]= "IVA";	$detalle[$i]["codigoRetencion"]= "";
                endif;
                ...
                $html.= "	<tr><td align='center' height='20'>".$datos_cmp["descripcion"]."</td>
												...
												<td align='center'>".substr($detalle[$i]["fechaEmisionDocSustento"],6,10)."</td>
					...
					<table style='width: 100%' align='center'>
							...
							<tr><td style='vertical-align:top; border: 1px; border-radius: 6px'>
								<table style='width: 100%'>
									<col style='width: 20%'>
									<col style='width: 80%'>
									<tr><td colspan='2'><p><b>INFORMACION ADICIONAL</b></p></td></tr>";
        $informacionAdicional= $arreglo["infoAdicional"]["campoAdicional"];
        ...
        if($this->tipo_doc=='01' or $this->tipo_doc=='04'):
            $html.= "		<tr><td>SUBTOTAL SIN IMPUESTOS</td>
										<td align='right'>".number_format($arreglo["infoCorreo"]["totalSinImpuestos"], 2, '.',',')."</td>
									</tr>";

            if($this->suma_imp["base_imp"]["2"]["2"]>=0):
                $html.= "		<tr><td>SUBTOTAL 12%</td>
										<td align='right'>".number_format($this->suma_imp["base_imp"]["2"]["2"], 2, '.',',')."</td>
									</tr>";
            elseif($this->suma_imp["base_imp"]["2"]["3"]>0):
                $html.= "		<tr><td>SUBTOTAL 14%</td>
										<td align='right'>".number_format($this->suma_imp["base_imp"]["2"]["3"], 2, '.',',')."</td>
									</tr>";
            endif;
            $html.= "		<tr><td>SUBTOTAL 0%</td>
									...
									";
            if($this->suma_imp["valor"]["2"]["2"]>=0):
                $html.= "		<tr><td>IVA 12%</td>
										<td align='right'>".number_format($this->suma_imp["valor"]["2"]["2"], 2, '.',',')."</td>
									</tr>";
            elseif($this->suma_imp["valor"]["2"]["3"]>0):
                $html.= "		<tr><td>IVA 14%</td>
										<td align='right'>".number_format($this->suma_imp["valor"]["2"]["3"], 2, '.',',')."</td>
									</tr>";
            endif;

            if($this->tipo_doc=='01' && false):
                $html.= "<tr><td>PROPINA</td>
										<td align='right'>".number_format($arreglo["infoCorreo"]["propina"], 2, '.',',')."</td>
									</tr>";
                if((float)$arreglo["infoCorreo"]["gastosExportacion"]>0):
                    $html.= "<tr><td>VALORES EXPORTACIÓN</td>
															<td align='right'>".number_format($arreglo["infoCorreo"]["gastosExportacion"], 2, '.',',')."</td>
														</tr>";
                endif;
            endif;
        endif;
        ...
        //pdf_create($html, "a4", "portrait", $output=FALSE, $mail_it=TRUE);
    }
    ...
    function envio_correo($infoCorreo)
    {
        ...
        $mail->MsgHTML($html);
        $envio_dest= false;
        if($this->tipo_ambiente==2):
            $address= str_replace(';',',',$infoCorreo["mail_to"]);
            $array_address= explode(",", $address);
            for($i=0; $i<count($array_address); $i++){
                $email= trim($array_address[$i]);
                if($this->valida_mail($email)){
                    $mail->AddAddress($email);	$envio_dest= true;
                }
            }
        endif;
    ...
    function ObtieneTipoIdentif($tipo_declaracion, $TipoIdentf, $Identf){
        $es_local= false;
        if(!in_array($TipoIdentf, array("CE", "PA", "INT"))):	$es_local= true;
        endif;
        if(!ereg("[^0-9]", $Identf) and strlen($Identf)==13 and $es_local):				$TipoIdentf="ruc";
        elseif(!ereg("[^0-9]", $Identf) and strlen($Identf)==10 and $es_local):			$TipoIdentf="cedula";
        elseif(!$es_local):																$TipoIdentf="pasaporte";
        endif;
    ...
    function DatosFactura($numeracion='', $sistema=''){
        $datos_emp= $this->querys_model->GetDatosEmpresa();
        $consulta_cabecera= $this->querys_model->GetDatosFactura($numeracion, $sistema);		$this->tipo_ambiente= $this->arr_parametros["tipo_ambiente"];	$i= 0;
        foreach($consulta_cabecera->result() as $row):											$tipoIdentif= $this->ObtieneTipoIdentif("ventas", $row->tipo_identif, $row->identif);
            ...
            $arreglo[$i]["infoTributaria"]["razonSocial"]= $datos_emp["n_empresa"];		$arreglo[$i]["infoTributaria"]["nombreComercial"]= $datos_emp["n_comercial"];
            $arreglo[$i]["infoTributaria"]["ruc"]= $datos_emp["ruc"];					$this->instancia= $row->instancia;
            ...
            //list($row->direccion, $row->cod_pais, $row->pais, $row->provincia, $row->ciudad, $row->tlf)= explode(":::::", $row->datos_direccion);
            $gastos_exportacion= $row->flete_internacional+$row->seguro_internacional+$row->gastos_aduaneros+$row->gastos_transportes_otros;

            if(trim($row->dir_estab) == "")			$row->dir_estab= " ";
            ...
            $arreglo[$i]["infoCorreo"]["propina"]= $row->propina;						$arreglo[$i]["infoCorreo"]["importeTotal"]= number_format($row->totalconimp+$row->propina+$gastos_exportacion, 2, '.','');
            ...
            $arreglo[$i]["infoCorreo"]["guiaRemision"]= $row->guia_remision;								$arreglo[$i]["infoCorreo"]["dirEstablecimiento"]= $row->dir_estab;
            $arreglo[$i]["infoCorreo"]["gastosExportacion"]= $gastos_exportacion;
            $arreglo[$i]["infoCorreo"]["direccionComprador"]= $row->direccion;;

            $arreglo[$i]["infoFactura"]["fechaEmision"]= $row->fechaemision;			$arreglo[$i]["infoFactura"]["dirEstablecimiento"]= $row->dir_estab;
            if($datos_emp["numero_contribuyente_especial"]!="")							$arreglo[$i]["infoFactura"]["contribuyenteEspecial"]= $datos_emp["numero_contribuyente_especial"];
            $arreglo[$i]["infoFactura"]["obligadoContabilidad"]= $datos_emp["obligado_contabilidad"];

            if($row->exportacion=="1"):				$arreglo[$i]["infoFactura"]["comercioExterior"]= "EXPORTADOR";									$arreglo[$i]["infoFactura"]["incoTermFactura"]= "FOB";
                $arreglo[$i]["infoFactura"]["lugarIncoTerm"]= $row->ciudad_estab."-".$row->pais_estab;			$arreglo[$i]["infoFactura"]["paisOrigen"]= $row->cod_pais_pe;
                $arreglo[$i]["infoFactura"]["puertoEmbarque"]= $row->puerto_embarque."-".$row->pais_pe;			$arreglo[$i]["infoFactura"]["puertoDestino"]= $row->puerto_destino."-".$row->pais_pd;
                $arreglo[$i]["infoFactura"]["paisDestino"]= $row->cod_pais_pd;									$arreglo[$i]["infoFactura"]["paisAdquisicion"]= $row->cod_pais_estab;
            endif;									$arreglo[$i]["infoFactura"]["tipoIdentificacionComprador"]= $tipoIdentif;
            if(trim($row->guia_remision)!=""):		$arreglo[$i]["infoFactura"]["guiaRemision"]= $row->guia_remision;
            endif;									$arreglo[$i]["infoFactura"]["razonSocialComprador"]= $row->n_cliente;			$arreglo[$i]["infoFactura"]["identificacionComprador"]= $row->identif;
            $arreglo[$i]["infoFactura"]["direccionComprador"]= $row->direccion;
            if($row->exportacion=="1"):				$arreglo[$i]["infoFactura"]["direccionComprador"]= $row->direccion;
            endif;									$arreglo[$i]["infoFactura"]["totalSinImpuestos"]= number_format($row->totalsinimp, 2, '.','');
            if($row->exportacion=="1"):				$arreglo[$i]["infoFactura"]["incoTermTotalSinImpuestos"]= "FOB";
            endif;									$arreglo[$i]["infoFactura"]["totalDescuento"]= number_format($row->totaldcto+$row->descuento_adicional, 2, '.','');
            ...
            for($xxx=0; $xxx<count($arr_codigoimp); $xxx++):			$codigoimp= $arr_codigoimp[$xxx];		$arr_codigoporc= array_keys($this->suma_imp["base_imp"][$codigoimp]);
                for($yyy=0; $yyy<count($arr_codigoporc); $yyy++):	$codigoporc= $arr_codigoporc[$yyy];
                    ...
                    $arreglo[$i]["infoFactura"]["totalConImpuestos"]["totalImpuesto"][$j]["valor"]= number_format($this->suma_imp["valor"][$codigoimp][$codigoporc], 2, '.','');		$j++;
                endfor;
            endfor;									$arreglo[$i]["infoFactura"]["propina"]= $row->propina;
            if($row->exportacion=="1"):				$arreglo[$i]["infoFactura"]["fleteInternacional"]= $row->flete_internacional;			$arreglo[$i]["infoFactura"]["seguroInternacional"]= $row->seguro_internacional;
                $arreglo[$i]["infoFactura"]["gastosAduaneros"]= $row->gastos_aduaneros;					$arreglo[$i]["infoFactura"]["gastosTransporteOtros"]= $row->gastos_transportes_otros;
            endif;									$arreglo[$i]["infoFactura"]["importeTotal"]= number_format($row->totalconimp+$row->propina+$gastos_exportacion, 2, '.','');
            $arreglo[$i]["infoFactura"]["moneda"]= "DOLAR";				$j=0; $f=0;
            ...
            foreach($consulta_detalle->result() as $rows):
                ...
                $arreglo[$i]["detalles"]["detalle"][$j]["descripcion"]= preg_replace( "/\r|\n/", "", substr(trim($rows->descripcion), 0, 300));
                ...
                if(trim($rows->unidad_medida)!=""):										$arreglo[$i]["detalles"]["detalle"][$j]["unidadMedida"]= $rows->unidad_medida;
                endif;
                ...
                if($row->cod_pais=="ECU"):					$rows->codigo_arancel= "";	$rows->peso_neto= "";		$rows->peso_bruto= "";
                else:										$rows->conversion= "";
                endif;										$var_marca= "";				$var_peso= "";

                $k= 0;
                if(trim($rows->codigo_arancel)!=""):		$atributos= array("nombre" => "Partida Arancelaria", "valor" => $rows->codigo_arancel);
                    $arreglo[$i]["detalles"]["detalle"][$j]["detallesAdicionales"]["detAdicional"][$k]["@attributes"]= $atributos;			$k++;
                    if(!in_array($atributos["nombre"], $this->det_adic_pdf) and count($this->det_adic_pdf)<$this->count_det_adic_pdf):		$this->det_adic_pdf[]= $atributos["nombre"];		endif;
                endif;
                if(trim($rows->marca)!=""):												$var_marca.= "$rows->marca";			endif;
                //if(trim($rows->lote)!=""):					if(trim($var_marca)!="")	$var_marca.= " / ";						$var_marca.= "Lt: $rows->lote";			endif;
                if(trim($var_marca)!=""):					$atributos= array("nombre" => "Marca", "valor" => $var_marca);
                    $arreglo[$i]["detalles"]["detalle"][$j]["detallesAdicionales"]["detAdicional"][$k]["@attributes"]= $atributos;			$k++;
                    if(!in_array($atributos["nombre"], $this->det_adic_pdf) and count($this->det_adic_pdf)<$this->count_det_adic_pdf):		$this->det_adic_pdf[]= $atributos["nombre"];		endif;
                endif;
                if(trim($rows->peso_neto)!=""):											$var_peso.= "PN: $rows->peso_neto ";	endif;
                if(trim($rows->peso_bruto)!=""):			if(trim($var_peso)!="")		$var_peso.= " / ";						$var_peso.= "PB: $rows->peso_bruto";	endif;
                if(trim($var_peso)!=""):					$atributos= array("nombre" => "Peso Neto / Bruto", "valor" => $var_peso);
                    $arreglo[$i]["detalles"]["detalle"][$j]["detallesAdicionales"]["detAdicional"][$k]["@attributes"]= $atributos;			$k++;
                    if(!in_array($atributos["nombre"], $this->det_adic_pdf) and count($this->det_adic_pdf)<$this->count_det_adic_pdf):		$this->det_adic_pdf[]= $atributos["nombre"];		endif;
                endif;
                if(trim($rows->conversion)!=""):			$atributos= array("nombre" => "Conversión", "valor" => $rows->conversion);
                    $arreglo[$i]["detalles"]["detalle"][$j]["detallesAdicionales"]["detAdicional"][$k]["@attributes"]= $atributos;			$k++;
                    if(!in_array($atributos["nombre"], $this->det_adic_pdf) and count($this->det_adic_pdf)<$this->count_det_adic_pdf):		$this->det_adic_pdf[]= $atributos["nombre"];		endif;
                endif;
            ...
            $j=0;
            if(trim($row->direccion)!=""):			$arreglo[$i]["infoAdicional"]["campoAdicional"][$j]["@attributes"]= array("nombre" => "Direccion");
                $arreglo[$i]["infoAdicional"]["campoAdicional"][$j]["@value"]= $row->direccion;			$j++;
            endif;
            if(trim($row->pais)!=""):				$arreglo[$i]["infoAdicional"]["campoAdicional"][$j]["@attributes"]= array("nombre" => "Ubicacion");
                $arreglo[$i]["infoAdicional"]["campoAdicional"][$j]["@value"]= $row->ciudad." - ".$row->provincia." (".$row->pais.")";			$j++;
            endif;
            if(trim($arreglo[$i]["infoFactura"]["puertoEmbarque"])!=""):	$arreglo[$i]["infoAdicional"]["campoAdicional"][$j]["@attributes"]= array("nombre" => "Puerto Embarque");
                $arreglo[$i]["infoAdicional"]["campoAdicional"][$j]["@value"]= preg_replace( "/\r|\n/", "", $arreglo[$i]["infoFactura"]["puertoEmbarque"]);		$j++;
            endif;
            if(trim($arreglo[$i]["infoFactura"]["puertoDestino"])!=""):		$arreglo[$i]["infoAdicional"]["campoAdicional"][$j]["@attributes"]= array("nombre" => "Puerto Destino");
                $arreglo[$i]["infoAdicional"]["campoAdicional"][$j]["@value"]= preg_replace( "/\r|\n/", "", $arreglo[$i]["infoFactura"]["puertoDestino"]);		$j++;
            endif;
            if(trim($row->embarcador)!=""):			//$arreglo[$i]["infoAdicional"]["campoAdicional"][$j]["@attributes"]= array("nombre" => "Trans. $row->tipo_emb");
                $arreglo[$i]["infoAdicional"]["campoAdicional"][$j]["@attributes"]= array("nombre" => "Naviera y Buque");
                $arreglo[$i]["infoAdicional"]["campoAdicional"][$j]["@value"]= preg_replace( "/\r|\n/", "", $row->embarcador);		$j++;
            endif;
            ...
            if(trim($row->observacion)!=""):		$arreglo[$i]["infoAdicional"]["campoAdicional"][$j]["@attributes"]= array("nombre" => "Observacion");
                $arreglo[$i]["infoAdicional"]["campoAdicional"][$j]["@value"]= preg_replace( "/\r|\n/", "", substr($row->observacion, 0, 300));		$j++;
            endif;
            ...
            if((float)$row->descuento_adicional>0):	$arreglo[$i]["infoAdicional"]["campoAdicional"][$j]["@attributes"]= array("nombre" => $row->n_descuento_adicional);
                $arreglo[$i]["infoAdicional"]["campoAdicional"][$j]["@value"]= preg_replace( "/\r|\n/", "", $row->descuento_adicional);		$j++;
            endif;
            if($row->exportacion!="1"):
            ...
            if($datos_emp["exportador"]==1):		$arreglo[$i]["infoAdicional"]["campoAdicional"][$j]["@attributes"]= array("nombre" => "Exportador Habitual");
                $arreglo[$i]["infoAdicional"]["campoAdicional"][$j]["@value"]= "SI";					$j++;
            endif;

            if(trim($row->plan)!=""):
                $arreglo[$i]["infoAdicional"]["campoAdicional"][$j]["@attributes"]= array("nombre" => "Plan");
                $arreglo[$i]["infoAdicional"]["campoAdicional"][$j]["@value"]= $row->plan;
                $j++;
            endif;

            if(trim($row->paciente_consulta)!="" && $row->paciente_consulta != null):
                $arreglo[$i]["infoAdicional"]["campoAdicional"][$j]["@attributes"]= array("nombre" => "Paciente");
                $arreglo[$i]["infoAdicional"]["campoAdicional"][$j]["@value"]= $row->paciente_consulta;
                $j++;
            endif;

            $agente_retencion = true;
            if($agente_retencion):
                $arreglo[$i]["infoAdicional"]["campoAdicional"][$j]["@attributes"]= array("nombre" => "CampoAdicional");
                $arreglo[$i]["infoAdicional"]["campoAdicional"][$j]["@value"]= "AGENTE DE RETENCION SEGUN RESOLUCION No. NAC-DGERCGC20-0000001";
                $j++;
            endif;
    ...
    function DatosNotaCredito($numeracion='', $sistema=''){
        ...
        foreach($consulta_cabecera->result() as $row):											$tipoIdentif= $this->ObtieneTipoIdentif("ventas", $row->tipo_identif, $row->identif);
            ...
            $arreglo[$i]["infoCorreo"]["totalSinImpuestos"]= number_format($row->totalsinimp, 2, '.','');	$arreglo[$i]["infoCorreo"]["contribuyenteEspecial"]= $datos_emp["numero_contribuyente_especial"];
            $arreglo[$i]["infoCorreo"]["totalDescuento"]= $row->totaldcto+$row->descuento_adicional;		$arreglo[$i]["infoCorreo"]["obligadoContabilidad"]= $datos_emp["obligado_contabilidad"];
            $arreglo[$i]["infoCorreo"]["guiaRemision"]= "";													$arreglo[$i]["infoCorreo"]["dirEstablecimiento"]= $row->dir_estab;

            $arreglo[$i]["infoNotaCredito"]["fechaEmision"]= $row->fechaemision;
            $arreglo[$i]["infoNotaCredito"]["dirEstablecimiento"]= $row->dir_estab;
            ...
            if($datos_emp["numero_contribuyente_especial"]!="")							$arreglo[$i]["infoNotaCredito"]["contribuyenteEspecial"]= $datos_emp["numero_contribuyente_especial"];
            ...
            $this->suma_imp= array();
            $consulta_detalle= $this->querys_model->GetDatosNotaCreditoDetalle($row->id, $row->tipo_nc, $sistema);
            $consulta_detalle_imp= $this->querys_model->GetDatosNotaCreditoDetalleImp($row->id, $row->tipo_nc, $sistema);
            if($row->totalconimp==$row->total_fac and $sistema=="SIIS"):
                $consulta_detalle= $this->querys_model->GetDatosFacturaDetalle($row->factura_id, $row->tipo_factura, $sistema);
            ...
            if(trim($row->observacion) == "")
                $row->observacion= "NN";
            $arreglo[$i]["infoNotaCredito"]["motivo"]= preg_replace( "/\r|\n/", "", substr($row->observacion, 0, 300));
            $j=0;

            foreach($consulta_detalle->result() as $rows):
                $arreglo[$i]["detalles"]["detalle"][$j]["codigoInterno"]= $rows->codigoprincipal;
                $arreglo[$i]["detalles"]["detalle"][$j]["descripcion"]= preg_replace( "/\r|\n/", "", substr(trim($rows->descripcion), 0, 300));
                if(trim($rows->unidad_medida)!=""):																	//$arreglo[$i]["detalles"]["detalle"][$j]["unidadMedida"]= $rows->unidad_medida;
                endif;
                $arreglo[$i]["detalles"]["detalle"][$j]["cantidad"]= $rows->cantidad;
                $arreglo[$i]["detalles"]["detalle"][$j]["precioUnitario"]= number_format($rows->precio, 2, '.','');
                $arreglo[$i]["detalles"]["detalle"][$j]["descuento"]= number_format($rows->descuento,2, '.','');
                $arreglo[$i]["detalles"]["detalle"][$j]["precioTotalSinImpuesto"]= number_format($rows->totalsinimp, 2, '.','');

                if($row->cod_pais=="ECU"):
                    $rows->codigo_arancel= "";	$rows->peso_neto= "";
                    $rows->peso_bruto= "";
                else:
                    $rows->conversion= "";
                endif;
                ...
                $k= 0;
                if(trim($rows->codigo_arancel)!=""):
                    $atributos= array("nombre" => "Partida Arancelaria", "valor" => $rows->codigo_arancel);
                    $arreglo[$i]["detalles"]["detalle"][$j]["detallesAdicionales"]["detAdicional"][$k]["@attributes"]= $atributos;
                    $k++;
                    if(!in_array($atributos["nombre"], $this->det_adic_pdf) and count($this->det_adic_pdf)<$this->count_det_adic_pdf):
                        $this->det_adic_pdf[]= $atributos["nombre"];
                    endif;
                endif;
                if(trim($rows->marca)!=""):
                    $var_marca.= "$rows->marca";
                endif;
                //if(trim($rows->lote)!=""):					if(trim($var_marca)!="")	$var_marca.= " / ";						$var_marca.= "Lt: $rows->lote";			endif;
                if(trim($var_marca)!=""):
                    $atributos= array("nombre" => "Marca", "valor" => $var_marca);
                    $arreglo[$i]["detalles"]["detalle"][$j]["detallesAdicionales"]["detAdicional"][$k]["@attributes"]= $atributos;
                    $k++;
                    if(!in_array($atributos["nombre"], $this->det_adic_pdf) and count($this->det_adic_pdf)<$this->count_det_adic_pdf):
                        $this->det_adic_pdf[]= $atributos["nombre"];
                ...
                if(trim($rows->peso_neto)!=""):
                    $var_peso.= "PN: $rows->peso_neto ";
                endif;
                if(trim($rows->peso_bruto)!=""):
                    if(trim($var_peso)!="")
                        $var_peso.= " / ";
                    $var_peso.= "PB: $rows->peso_bruto";
                endif;
                if(trim($var_peso)!=""):
                    $atributos= array("nombre" => "Peso Neto / Bruto", "valor" => $var_peso);
                    $arreglo[$i]["detalles"]["detalle"][$j]["detallesAdicionales"]["detAdicional"][$k]["@attributes"]= $atributos;
                    $k++;
                    if(!in_array($atributos["nombre"], $this->det_adic_pdf) and count($this->det_adic_pdf)<$this->count_det_adic_pdf):
                        $this->det_adic_pdf[]= $atributos["nombre"];
                ...
                for($k=0; $k<count($arr_imp[$rows->det_id]["codigoimp"]); $k++):
                    if($arr_imp[$rows->det_id]["codigoimp"][$k]==""):
                        $arr_imp[$rows->det_id]["codigoimp"][$k]= "2";
                        $arr_imp[$rows->det_id]["codigoporc"][$k]= "0";
                    endif;
                    $arreglo[$i]["detalles"]["detalle"][$j]["impuestos"]["impuesto"][$k]["codigo"]= $arr_imp[$rows->det_id]["codigoimp"][$k];
                    $arreglo[$i]["detalles"]["detalle"][$j]["impuestos"]["impuesto"][$k]["codigoPorcentaje"]= $arr_imp[$rows->det_id]["codigoporc"][$k];
            ...
            if(trim($row->direccion)!=""):
                $arreglo[$i]["infoAdicional"]["campoAdicional"][$j]["@attributes"]= array("nombre" => "Direccion");
                $arreglo[$i]["infoAdicional"]["campoAdicional"][$j]["@value"]= $row->direccion;
                $j++;
            endif;
            if(trim($row->pais)!=""):
                $arreglo[$i]["infoAdicional"]["campoAdicional"][$j]["@attributes"]= array("nombre" => "Ubicacion");
                $arreglo[$i]["infoAdicional"]["campoAdicional"][$j]["@value"]= $row->ciudad." - ".$row->provincia." (".$row->pais.")";			$j++;
            endif;
            if(trim($row->tlf)!=""):
                $arreglo[$i]["infoAdicional"]["campoAdicional"][$j]["@attributes"]= array("nombre" => "Telefono");
                $arreglo[$i]["infoAdicional"]["campoAdicional"][$j]["@value"]= $row->tlf;
                $j++;
            endif;
            if(trim($row->email)!=""):
                $arreglo[$i]["infoAdicional"]["campoAdicional"][$j]["@attributes"]= array("nombre" => "Email");
                $arreglo[$i]["infoAdicional"]["campoAdicional"][$j]["@value"]= $row->email;
                $j++;
            endif;
            /*if(trim($row->observacion)!=""):		$arreglo[$i]["infoAdicional"]["campoAdicional"][$j]["@attributes"]= array("nombre" => "Observaci�n");
                                                    $arreglo[$i]["infoAdicional"]["campoAdicional"][$j]["@value"]= preg_replace( "/\r|\n/", "", substr($row->observacion, 0, 300));		$j++;
            endif;
            if(trim($row->ordcompra_cliente)!=""):	$arreglo[$i]["infoAdicional"]["campoAdicional"][$j]["@attributes"]= array("nombre" => "Orden de Compra Cliente");
                                                    $arreglo[$i]["infoAdicional"]["campoAdicional"][$j]["@value"]= $row->ordcompra_cliente;		$j++;
            endif;*/
            if($datos_emp["exportador"]==1):
                $arreglo[$i]["infoAdicional"]["campoAdicional"][$j]["@attributes"]= array("nombre" => "Exportador Habitual");
                $arreglo[$i]["infoAdicional"]["campoAdicional"][$j]["@value"]= "SI";
                $j++;
            endif;

            $agente_NCretencion = true;
            if($agente_NCretencion):
                $arreglo[$i]["infoAdicional"]["campoAdicional"][$j]["@attributes"]= array("nombre" => "CampoAdicional");
                $arreglo[$i]["infoAdicional"]["campoAdicional"][$j]["@value"]= "AGENTE DE RETENCION SEGUN RESOLUCION No. NAC-DGERCGC20-0000001";
                $j++;
            endif;
    ...
    function DatosGuiaRemision($numeracion='', $sistema=''){
        ...
        foreach($consulta_cabecera->result() as $row):					list($row->direccion, $row->cod_pais, $row->pais, $row->provincia, $row->ciudad, $row->tlf)= explode(":::::", $row->datos_direccion);
            if($row->identif==""):								$row->tipo_identif= "RUC";
                $row->identif= $datos_emp["ruc"];			$row->n_cliente= $datos_emp["n_empresa"];
                $row->direccion= $row->dir_estab;
            ...
            if($row->ruc_transportista=="")		$row->ruc_transportista= "9999999999";
            ...
            $arreglo[$i]["infoCorreo"]["dirEstablecimiento"]= $row->dir_estab;			$arreglo[$i]["infoCorreo"]["contribuyenteEspecial"]= $datos_emp["numero_contribuyente_especial"];
            $arreglo[$i]["infoCorreo"]["obligadoContabilidad"]= $datos_emp["obligado_contabilidad"];

            $arreglo[$i]["infoGuiaRemision"]["dirEstablecimiento"]= $row->dir_estab;
            $arreglo[$i]["infoGuiaRemision"]["dirPartida"]= $row->dir_estab;
            ...
            if($datos_emp["numero_contribuyente_especial"]!="")							$arreglo[$i]["infoGuiaRemision"]["contribuyenteEspecial"]= $datos_emp["numero_contribuyente_especial"];
            ...
            foreach($consulta_detalle->result() as $rows):		$arreglo[$i]["destinatarios"]["destinatario"]["detalles"]["detalle"][$j]["codigoInterno"]= $rows->codigoprincipal;
                $arreglo[$i]["destinatarios"]["destinatario"]["detalles"]["detalle"][$j]["descripcion"]= $rows->descripcion;
                if(trim($rows->unidad_medida)!=""):			$arreglo[$i]["destinatarios"]["destinatario"]["detalles"]["detalle"][$j]["unidadMedida"]= $rows->unidad_medida;
                endif;										$arreglo[$i]["destinatarios"]["destinatario"]["detalles"]["detalle"][$j]["cantidad"]= $rows->cantidad;
    ...
    function DatosRetencion($numeracion='', $sistema=''){
        ...
        foreach($consulta_cabecera->result() as $row):											$tipoIdentif= $this->ObtieneTipoIdentif("ventas", $row->tipo_identif, $row->identif);
            ...
            $arreglo[$i]["infoCorreo"]["dirEstablecimiento"]= $row->dir_estab;			$arreglo[$i]["infoCorreo"]["contribuyenteEspecial"]= $datos_emp["numero_contribuyente_especial"];
            $arreglo[$i]["infoCorreo"]["obligadoContabilidad"]= $datos_emp["obligado_contabilidad"];
            //DIRECCION CORREO ELECTRONICO PROVEEDOR
            $arreglo[$i]["infoCorreo"]["direccionComprador"]= $row->direccion_pto;

            $arreglo[$i]["infoCompRetencion"]["fechaEmision"]= $row->fechaemision;		$arreglo[$i]["infoCompRetencion"]["dirEstablecimiento"]= $row->dir_estab;
            if($datos_emp["numero_contribuyente_especial"]!="")							$arreglo[$i]["infoCompRetencion"]["contribuyenteEspecial"]= $datos_emp["numero_contribuyente_especial"];
            ...
            $agente_Retretencion = true;
            if($agente_Retretencion):
                $arreglo[$i]["infoAdicional"]["campoAdicional"][$j]["@attributes"]= array("nombre" => "CampoAdicional");
                $arreglo[$i]["infoAdicional"]["campoAdicional"][$j]["@value"]= "AGENTE DE RETENCION SEGUN RESOLUCION No. NAC-DGERCGC20-0000001";
                $j++;
            endif;
